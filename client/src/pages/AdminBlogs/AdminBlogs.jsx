...
function AdminBlog() {
    ...
    const fetchPosts = async () => {
        try {
            const response = await fetch('https://13.60.226.150:5000/api/blogs');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingPostId) {
                await fetch(`https://13.60.226.150:5000/api/blogs/${editingPostId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(formData),
                });
            } else {
                await fetch('https://13.60.226.150:5000/api/blogs', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(formData),
                });
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`https://13.60.226.150:5000/api/blogs/${id}`, {
                method: 'DELETE',
            });
