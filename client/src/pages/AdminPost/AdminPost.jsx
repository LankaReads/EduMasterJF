...
function AdminPosts() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingPostId) {
                await fetch(`http://13.60.226.150:5000/api/posts/${editingPostId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(formData),
                });
            } else {
                await fetch('http://13.60.226.150:5000/api/posts', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(formData),
                });
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`http://13.60.226.150:5000/api/posts/${id}`, {
                method: 'DELETE',
            });
